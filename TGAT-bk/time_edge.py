...
def eval_one_epoch(hint, tgan, src, dst, ts, label):
    ...
    for k in trange(num_test_batch):
        ...
        cut_time_l = ts_l_cut
        for i in range(2):
            src_idx_l, _, cut_time_l = ngh_finder.get_temporal_neighbor(
                src_idx_l, cut_time_l, num_neighbors=20)
            src_idx_l = src_idx_l.flatten()
            cut_time_l = cut_time_l.flatten()
        
        dst_idx_l = dst_l_cut
        cut_time_l = ts_l_cut
        for i in range(2):
            dst_idx_l, _, cut_time_l = ngh_finder.get_temporal_neighbor(
                dst_idx_l, cut_time_l, num_neighbors=20)
            dst_idx_l = dst_idx_l.flatten()
            cut_time_l = cut_time_l.flatten()
        # prob_score = tgan.forward(src_l_cut, dst_l_cut, ts_l_cut).sigmoid()
    end = time.time()
    return num_test_batch, end - start
...
train_label_l = np.ones_like(idx_list)
time_l = []

for epoch in trange(NUM_EPOCH):

    _, duration = eval_one_epoch('train time', tgan, train_src_l,
                                         train_dst_l, train_ts_l,
                                         train_label_l)
    time_l.append(duration)

time_epoch = np.mean(time_l)
time_batch = time_epoch / num_batch
res_path = "time_measure/Time-{}-tgat.csv".format(DATA)
headers = ["method", "dataset", "time_batch", "time_epoch", "params"]
...
with open(res_path, "a") as file:
    file.write("tgat,{},{:.4f},{:.4f},\"{}\"".format(
        DATA, time_batch, time_epoch, params))
    file.write("\n")
...
res_path = "result/{}-tgat.csv".format(DATA)
...
with open(res_path, "a") as file:
    file.write("tgat,{},{:.4f},{:.4f},{:.4f},{:.4f},\"{}\"".format(
